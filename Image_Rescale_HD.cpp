...
//	register unsigned char RT1, GT1, BT1, RT2, GT2, BT2, empty5, empty6;
	...
	pp = (unsigned char *)calloc(24, sizeof(unsigned char));
	...
	register unsigned char *qq = pp + 8;

	register unsigned char *xx = pp + 16;
...
//	&RT1; &GT1; &BT1; &RT2; &GT2; &BT2; &empty5; &empty6;
//	asm("kkk");
...
//	printf("0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x\n", &R3, &G3, &B3, &R4, &G4, &B4, &RT2, &GT2);
	...
	for (y = 0; y != dest_y; ++ y)
	{
		...
		if (last != fl_y)
		{
			...
			*(pp + 16) = f[dx[0]][*(pp + 3)] + f[temp_x[0]][*pp];
			*(pp + 17) = f[dx[0]][*(pp + 4)] + f[temp_x[0]][*(pp + 1)];
			*(pp + 18) = f[dx[0]][*(pp + 5)] + f[temp_x[0]][*(pp + 2)];
			*(pp + 19) = f[dx[0]][*(pp + 11)] + f[temp_x[0]][*(pp + 8)];
			*(pp + 20) = f[dx[0]][*(pp + 12)] + f[temp_x[0]][*(pp + 9)];
			*(pp + 21) = f[dx[0]][*(pp + 13)] + f[temp_x[0]][*(pp + 10)];
			
			rec[0].k = *((long long*)xx);
			...
			for (x = 1; x != dest_x; ++ x)
			{
				...
				*(pp + 16) = f[dx[x]][*(pp + 3)] + f[temp_x[x]][*pp];
				*(pp + 17) = f[dx[x]][*(pp + 4)] + f[temp_x[x]][*(pp + 1)];
				*(pp + 18) = f[dx[x]][*(pp + 5)] + f[temp_x[x]][*(pp + 2)];
				*(pp + 19) = f[dx[x]][*(pp + 11)] + f[temp_x[x]][*(pp + 8)];
				*(pp + 20) = f[dx[x]][*(pp + 12)] + f[temp_x[x]][*(pp + 9)];
				*(pp + 21) = f[dx[x]][*(pp + 13)] + f[temp_x[x]][*(pp + 10)];
				rec[x].k = *((long long*)xx);
			}
			q += delta;
		...
		for (x = 0; x != dest_x; ++ x)
		{
			*offset ++ = f[temp_y][rec[x].c[0]] + f[dy][rec[x].c[3]];
			*offset ++ = f[temp_y][rec[x].c[1]] + f[dy][rec[x].c[4]];
			*offset ++ = f[temp_y][rec[x].c[2]] + f[dy][rec[x].c[5]];
		}
		fy += step_y;
		last = fl_y;
	}
