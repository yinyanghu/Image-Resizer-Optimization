...
//	printf("0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x\n", &R3, &G3, &B3, &R4, &G4, &B4, &RT2, &GT2);
	...
	for (y = 0; y != dest_y; ++ y)
	{
		...
		*(offset + 1) = f[dx[0]][f[dy][GT1] + GT2] + f[dy][GT3] + G1;
		*(offset + 2) = f[dx[0]][f[dy][BT1] + BT2] + f[dy][BT3] + B1;
		offset += 3;
...
//		*offset ++ = (B1 + B4 - B2 - B3) * dy * dx[0] + (B2 - B1) * dx[0] + (B3 - B1) * dy + B1;
		...
		for (x = 1; x != dest_x; ++ x)
		{
			...
			*/
			...
			//left = f[dx[x]];
			//*offset = *(left + *(right + RT1) + RT2) + *(right + RT3) + R1;
			//*(offset + 1) = *(left + *(right + GT1) + GT2) + *(right + GT3) + G1;
			//*(offset + 2) = *(left + *(right + BT1) + BT2) + *(right + BT3) + B1;
			*offset = f[dx[x]][f[dy][RT1] + RT2] + f[dy][RT3] + R1;
			*(offset + 1) = f[dx[x]][f[dy][GT1] + GT2] + f[dy][GT3] + G1;
			*(offset + 2) = f[dx[x]][f[dy][BT1] + BT2] + f[dy][BT3] + B1;
			offset += 3;
		}
